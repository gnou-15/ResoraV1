...
function AIScoreWidget({ resume, profession, analysisResult, loading, onUpdateResume, moodOverride }) {
  ...
  const strokeDashoffset = strokeCircumference * (1 - Math.min(100, Math.max(0, score)) / 100);
  ...
  const bannerText = loading
    ? "Evaluating Resume with AI Scorer..."
    : "ATS & Talent Insights Engine Active";

  const badgeText = loading ? "Scanning..." : "Synced";
    ...
    <div className={cardClasses}>
      ...
      {loading ? (
        <div className="ai-widget-loading-body animate-fade-in">
          ...
          <div className="sync-blinking-text">Evaluating Resume...</div>
        </div>
      ) : (
        <>
          {/* Analysis Body */}
          <div className="ai-widget-body animate-fade-in">
            ...
            <div className="score-and-mascot-panel">
              ...
              <div className="mascot-panel">
                <div className="sync-dance-wrapper">
                  <PeekingMonster mood={moodOverride || mascotMood} />
                </div>
            ...
            <div className="placement-details-panel">
              ...
              <p className="placement-desc">
                Estimated accept rate: <strong>{acceptancePercentage}%</strong> on ATS &amp; job portals.
              </p>

              <div className="mini-breakdown">
                <div className="breakdown-item">
                  <span className="b-label">Contact details</span>
                  <div className="b-bar"><div className="b-fill" style={{ width: `${Math.min(100, Math.max(0, (scoreBreakdown.contact/15)*100))}%`, background: placementColor }}></div></div>
                </div>
                <div className="breakdown-item">
                  <span className="b-label">Headline &amp; Summary</span>
                  <div className="b-bar"><div className="b-fill" style={{ width: `${Math.min(100, Math.max(0, ((scoreBreakdown.headline + scoreBreakdown.summary)/25)*100))}%`, background: placementColor }}></div></div>
                </div>
                <div className="breakdown-item">
                  <span className="b-label">Skills alignment</span>
                  <div className="b-bar"><div className="b-fill" style={{ width: `${Math.min(100, Math.max(0, (scoreBreakdown.skills/20)*100))}%`, background: placementColor }}></div></div>
                </div>
                <div className="breakdown-item">
                  <span className="b-label">Experience impact</span>
                  <div className="b-bar"><div className="b-fill" style={{ width: `${Math.min(100, Math.max(0, (scoreBreakdown.experience/25)*100))}%`, background: placementColor }}></div></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="ai-widget-suggestions animate-fade-in">
            ...
            >
              <span>{expanded ? "Hide Recommendations" : `Show ATS Recommendations (${tips.length} action items)`}</span>
            ...
            <div 
              ...
              style={{
                ...
                opacity: expanded ? (loading ? 0.55 : 1) : 0,
                padding: expanded ? '1.25rem' : '0px 1.25rem',
                overflow: 'hidden',
            ...
            >
              {tips.length === 0 ? (
                <div className="all-clear-msg">
                  🎉 Perfect! Your resume scores 100% and meets all top ATS benchmarks.
                </div>
              ) : (
